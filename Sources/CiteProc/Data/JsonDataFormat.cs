...
internal class JsonDataFormat : DataFormat
{
    ...
    private static DateVariable? ParseRawDate(IO.JsonReader jr)
    {
        ...
        {
            Match yearMatch = Regex.Match(text, @"\d{4}");
            if (yearMatch.Success)
            {
                int year = int.Parse(yearMatch.Value);
                int? month = null;

                // Logic to find a month (1-2 digits) that isn't the year
                var monthMatch = Regex.Match(text, @"\b(?<!\d{2})\d{1,2}\b");
                if (monthMatch.Success && monthMatch.Value != yearMatch.Value)
                {
                    int m = int.Parse(monthMatch.Value);
                    if (m >= 1 && m <= 12)
                    {
                        month = m;
                    }
                ...
                result = new DateVariable(year, null, month, null, false);
            }
            else
            {
                throw new DataFormatException(jr, "Invalid raw date value '{0}'.", text);
            }
        }
